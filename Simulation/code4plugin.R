...
## oracle selection 
##simulation setting 1£ºnot overlapping 2. partially overlapping 3.the same region
    ...
    for (tauX in tauX_list)
    {
      ...
      #    causalsnps=c(causalsnps,ind3)
       #   alpha[ind3]=rnorm(length(ind3),0,sd=sqrt(sigma2y))
        ...
        #eleY1=sum(betax[indx]*betahat_y[indx]/(se_y[indx]^2))
        #eleY2=sum(betam[indm]*betahat_y[indm]/(se_y[indm]^2))
        #matrix
        ...
        # regression_residuals1= sum( (betahat_y[indx] *over2$gamma_exp1[indx]-thetaRIVW * (over2$gamma_exp1[indx]^2 - over2$se1[indx]^2) -tauyRIVW*over2$gamma_exp1[indx]*over2$gamma_exp2[indx])^2/ se_y[indx]^4)
        #  regression_residuals2= sum( (betahat_y[indm] *over2$gamma_exp2[indm]-tauyRIVW * (over2$gamma_exp2[indm]^2 - over2$se2[indm]^2) -thetaRIVW*over2$gamma_exp2[indm]*over2$gamma_exp1[indm])^2/ se_y[indm]^4)
        # regression_residuals3= sum( (betahat_m[indx] * betanew_x - tauxRIVW * (betanew_x ^2 - senew_x^2))^2 / se_m[indx]^4)
        ...
        cov_12= sum( (betahat_y[overlap_index] * betanew_x[overlap_x]-thetaRIVW * (betanew_x[overlap_x] ^2 - senew_x[overlap_x]^2) -tauyRIVW*betanew_x[overlap_x]*betanew_m[overlap_m])*(betahat_y[overlap_index] * betanew_m[overlap_m] -tauyRIVW * (betanew_m[overlap_m]^2 - senew_m[overlap_m]^2) -thetaRIVW*betanew_x[overlap_x]*betanew_m[overlap_m])/ se_y[overlap_index]^4)
        #cov_12=sum((betanew_m[overlap_m]*betanew_x[overlap_x]*(se_y[overlap_index]^2+thetaRIVW^2*se_x[overlap_index]^2+tauyRIVW^2*se_m[overlap_index]^2))/se_y[overlap_index]^4)#+tauyRIVW*thetaRIVW*sum(se_x[overlap_index]^2*se_m[overlap_index]^2/se_y[overlap_index]^4)
        #cov_12=sum((betahat_x[overlap_index]^2-se_x[overlap_index]^2)*(se_y[overlap_index]^2+thetaRIVW^2*se_x[overlap_index]^2+tauyRIVW^2*se_m[overlap_index]^2)/se_y[overlap_index]^4)+tauyRIVW*thetaRIVW*sum(se_x[overlap_index]^2*se_m[overlap_index]^2/se_y[overlap_index]^4)
        #cov_13=sum( (betahat_y[overlap_index] * betanew_x[overlap_x]-thetaRIVW * (betanew_x[overlap_x] ^2 - senew_x[overlap_x]^2) -tauyRIVW*betanew_x[overlap_x]*over2$gamma_exp2[overlap_index])*(betahat_m[overlap_index] * betanew_x[overlap_x] - tauxRIVW * (betanew_x[overlap_x] ^2 - senew_x[overlap_x]^2))/ (se_y[overlap_index]^2*se_m[overlap_index]^2))+sum( (betahat_y[setdiff(indx,overlap_index)] *  over2$gamma_exp1[setdiff(indx,overlap_index)]-thetaRIVW * (over2$gamma_exp1[setdiff(indx,overlap_index)] ^2 - over2$se1[setdiff(indx,overlap_index)]^2) -tauyRIVW* over2$gamma_exp1[setdiff(indx,overlap_index)]*over2$gamma_exp2[setdiff(indx,overlap_index)])*(betahat_m[setdiff(indx,overlap_index)] *over2$gamma_exp1[setdiff(indx,overlap_index)] - tauxRIVW * (over2$gamma_exp1[setdiff(indx,overlap_index)] ^2 - over2$se1[setdiff(indx,overlap_index)]^2))/ (se_y[setdiff(indx,overlap_index)]^2*se_m[setdiff(indx,overlap_index)]^2))
        ...
        #cov_13=sum((betanew_x^2-senew_x^2)*(thetaRIVW*tauxRIVW*senew_x^2-tauyRIVW*se_m[indx]^2)/(se_y[indx]^2*se_m[indx]^2))
        #cov_23=sum(tauxRIVW*(betahat_x[overlap_index]^2-se_x[overlap_index]^2)*(thetaRIVW*tauxRIVW*se_x[overlap_index]^2-tauyRIVW*se_m[overlap_index]^2)/(se_y[overlap_index]^2*se_m[overlap_index]^2))-thetaRIVW*sum(se_x[overlap_index]^2/se_y[overlap_index]^2)86
        ...
        #cov_23=sum((betanew_m[overlap_m]*betanew_x[overlap_x])*(thetaRIVW*tauxRIVW*se_x[overlap_index]^2-tauyRIVW*se_m[overlap_index]^2)/(se_y[overlap_index]^2*se_m[overlap_index]^2))-thetaRIVW*sum(se_x[overlap_index]^2/se_y[overlap_index]^2)
        #regression_residuals1= sum( (betahat_y[indx] * betanew_x -theta * (betanew_x ^2 - senew_x^2) -tauY*betanew_x*betahat_m[indx])^2/ se_y[indx]^4)
        #regression_residuals2= sum( (betahat_y[indm] * betanew_m -tauY * (betanew_m ^2 - senew_m^2) -theta*betahat_x[indm]*betanew_m)^2/ se_y[indm]^4)
        #regression_residuals3= sum( (betahat_m[indx] * betanew_x - tauxRIVW * (betanew_x ^2 - senew_x^2))^2 / se_m[indx]^4)
        #cov_12=sum((betax[overlap_index]*betam[overlap_index])*(se_y[overlap_index]^2+theta^2*se_x[overlap_index]^2+tauY^2*se_m[overlap_index]^2)/se_y[overlap_index]^4)+tauY*theta*sum(se_x[overlap_index]^2*se_m[overlap_index]^2/se_y[overlap_index]^4)
        #cov_13=sum((betanew_x^2-senew_x^2)*(theta*tauX*senew_x^2-tauY*se_m[indx]^2)/(se_y[indx]^2*se_m[indx]^2))
        #cov_23=sum((betax[overlap_index]*betam[overlap_index])*(theta*tauX*se_x[overlap_index]^2-tauY*se_m[overlap_index]^2)/(se_y[overlap_index]^2*se_m[overlap_index]^2))-theta*sum(se_x[overlap_index]^2/se_y[overlap_index]^2)
        # corr_list=c(corr_list,cor(over2$gamma_exp1[overlap_index],over2$gamma_exp2[overlap_index]))
        ...
        #v4_tau=vector_C%*%sub_variance_matrix%*%(vector_C)
        ...
        ele4=sum((betanew_m^2-senew_m^2)/(se_y[indm]^2))
        ele5=sum((betanew_x^2-senew_x^2)/(se_m[indx]^2))
        ele2=sum(over2$gamma_exp1[overlap_index]*over2$gamma_exp2[overlap_index]/(se_y[overlap_index]^2))
        ele3=sum(over2$gamma_exp1[overlap_index]*over2$gamma_exp2[overlap_index]/(se_y[overlap_index]^2))
        eleY1=sum(betanew_x*betahat_y[indx]/(se_y[indx]^2))
        eleY2=sum(betanew_m*betahat_y[indm]/(se_y[indm]^2))
        eleY3=sum(betanew_x[overlap_x]*over2$gamma_exp2[overlap_index]/(se_m[overlap_index]^2))+sum(over2$gamma_exp1[setdiff(indx,overlap_index)]*over2$gamma_exp2.carve[setdiff(indx,overlap_index)]/(se_m[setdiff(indx,overlap_index)]^2))
        ...
        sum_vector=c(eleY1,eleY2,eleY3)
        estimator_rerand=solve(sum_mat)%*%sum_vector
        thetaOptimal=estimator_rerand[1]
        tauyOptimal=estimator_rerand[2]
        tauxOptimal=estimator_rerand[3]
        tauOptimal=tauyOptimal*tauxOptimal
  
        theta_Optimallist=c(theta_Optimallist,thetaOptimal)
        tauY_Optimallist=c(tauY_Optimallist,tauyOptimal)
        tauX_Optimallist=c(tauX_Optimallist,tauxOptimal)
        tau_Optimallist=c(tau_Optimallist,tauOptimal)
        print(i)
      }
      
      result=data.frame(cbind(theta_rerandlist,theta_rerandse,tauX_rerandlist,tauX_rerandse,tauY_rerandlist,tauY_rerandse,tau_rerandlist,tau_rerandse,theta_Optimallist,tauX_Optimallist,tauY_Optimallist,tau_Optimallist,mvmr_theta,mvmr_tauX,mvmr_tauY,mvmr_tau,mvmr_theta_ivw,ivw_theta_se,mvmr_tauX_ivw,ivw_tauX_se,mvmr_tauY_ivw,ivw_tauY_se,mvmr_tau_ivw,twostep_tauX,twostep_tauX_sd,twostep_tauY,twostep_tauY_sd,twostep_tau,twostep_tau_sd,total_effects_list,total_effects_sd,difference_tau_list))
      colnames(result)=c("thetarerand","thetase","tauxrerand","tauxse","tauyrerand","tauyse","taurerand","tause","thetaOptimal","tauxOptimal","tauyOptimal","tauOptimal","mvmr_theta","mvmr_tauX","mvmr_tauY","mvmr_tau","mvmrIVW_theta","mvmrIVW_theta_se","mvmrIVW_tauX","mvmrIVW_tauX_se","mvmrIVW_tauY","mvmrIVW_tauY_se","mvmrIVW_tau","2step_tauX","2step_tauX_sd","2step_tauY","2step_tauY_sd","2step_tau","2step_tau_sd","total_effect","total_effect_sd","difference_tau")
      estimation=data.frame(cbind(theta_rerandlist,tauX_rerandlist,tauY_rerandlist,tau_rerandlist,theta_Optimallist,tauX_Optimallist,tauY_Optimallist,tau_Optimallist,mvmr_theta,mvmr_tauX,mvmr_tauY,mvmr_tau,mvmr_theta_ivw,mvmr_tauX_ivw,mvmr_tauY_ivw,mvmr_tau_ivw,twostep_tauX,twostep_tauY,twostep_tau,total_effects_list,difference_tau_list))
      ...
      #cover_theta_Optimal=cover(theta_Optimallist,theta_Optimalse,theta)
      #cover_tauX_Optimal=cover(tauX_Optimallist,tauX_Optimalse,tauX)
      #cover_tauY_Optimal=cover(tauY_Optimallist,tauY_Optimalse,tauY)
      #cover_tau_Optimal=cover(tau_Optimallist,tau_Optimalse,tau)
     ...
     # abias_theta_Optimal=abias(theta_Optimallist,theta)
    #  abias_tauX_Optimal=abias(tauX_Optimallist,tauX)
     # abias_tauY_Optimal=abias(tauY_Optimallist,tauY)
    #  abias_tau_Optimal=abias(tau_Optimallist,tau)
      ...
      save.image(paste0("D:\\Research\\structural equation\\Optimal_comparison\\NOVsimudesign1pi1=0.01pi3=0.010.5e10-41e-4theta=",theta,"tauY=",tauY,"tauX=",tauX,"M=",M,".Rdata"))
